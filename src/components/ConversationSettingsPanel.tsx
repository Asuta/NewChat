...
export function ConversationSettingsPanel({
  ...
  const [value, setValue] = useState(conversation.fixedContext?.content || '');

  useEffect(() => {
    setValue(conversation.fixedContext?.content || '');
  }, [conversation.id, conversation.fixedContext?.content]);

  const trimmed = value.trim();
  const savedContent = conversation.fixedContext?.content.trim() || '';
  const hasFixedContext = Boolean(savedContent);
  const hasDynamicChat = conversation.messages.length > 0 || Boolean(conversation.contextSummary);
  const canSave = !disabled && trimmed !== savedContent;
  ...
  return (
    <div className="settings-panel" role="dialog" aria-label="会话设置">
      ...
      <label className="fixed-context-field">
        ...
        <textarea
          aria-label="固定上下文"
          placeholder="输入此会话每次都需要携带的固定背景、角色设定、约束或长期目标。"
          rows={8}
          value={value}
          disabled={disabled}
      ...
      <div className="settings-actions">
        <button className="settings-primary" type="button" disabled={!canSave} onClick={() => onSaveFixedContext(trimmed)}>
          <Save size={16} />
          保存固定上下文
        </button>
      ...
      <div className="settings-danger-zone">
        <div>
          ...
          <span>只清空动态消息和压缩摘要，固定上下文会保留。</span>
        </div>
