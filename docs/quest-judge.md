...
- 游戏舞台左侧常驻显示最多 4 条进行中任务；点击“当前任务”或“查看全部任务”可展开完整任务日志，查看全部进行中、未开始、已完成和已失败任务。点击任意一条任务会展开该任务的说明、当前进度和达成条件；新的 `quest.progressed`、`quest.completed`、`quest.failed` 或 `quest.activated` 事件会触发更新提示。
