...
test('预制世界按第一集检查点提供完整且自洽的马大帅人物、场景和剧情数据', () => {
  ...
  assert.equal(meta.get(QUEST_JUDGE_INITIALIZED_META_KEY), 'ready');
  assert.equal(meta.get(QUEST_JUDGE_CONVERSATION_CURSOR_META_KEY), '0');
  assert.equal(meta.get(QUEST_JUDGE_EVENT_CURSOR_META_KEY), '7');
  ...
  const surviveQuest = components.get('quest_survive_city:quest');
  const findDebiaoQuest = components.get('quest_find_debiao:quest');
  const findXiaocuiQuest = components.get('quest_find_xiaocui:quest');
  ...
  assert.deepEqual(findXiaocuiQuest.onComplete.activateQuestIds, ['quest_cancel_marriage']);
  assert.equal(findXiaocuiQuest.status, 'inactive');
  assert.equal(findXiaocuiQuest.phaseStatus, 'hidden');
